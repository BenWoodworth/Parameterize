...
class ParameterizeConfigurationSpec {
    ...
    /**
     * The parameterize block and `decorator` should execute together, since it should work the same as if the
     * `decorator` logic were inlined into the block. Logic at the end of the block would still be executed before
     * `onFailure` and `onComplete` are invoked.
     *
     * Having `onFailure` run *after* the `decorator` is also easier to reason about if the handler itself throws. The
     * `decorator` has "always completes" semantics, and `onFailure` has "terminates immediately" semantics, and that's
     * only possible if `onFailure` runs after. If `onFailure` were to run immediately for a failure in the parameterize
     * block, then the `decorator`'s semantics with how it interacts with failures wouldn't be so straightforward.
     */
    @Test
